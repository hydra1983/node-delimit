...
var fs = require('fs')
, when = require('when')
, rowStreamer = require('../src/rowStreamer')
...
describe('rowStreamer', function() {
	...
	describe('#isDataRowEmpty()', function() {
		...
		it('should detect that the data row is empty', function() {
			...
			rowStreamer.isDataRowEmpty(datasetTransformer, dataRow).should.be.true;
		});

		it('should not detect that the data row is empty', function() {
			var dataRow = ['   ', '', '', 'not empty after all!'];
			rowStreamer.isDataRowEmpty(datasetTransformer, dataRow).should.be.false;
		});
	...
	describe('#toRows()', function() {
		...
		it('take a tsv file path and get the rows it contains', function() {
			...
			return rowStreamer.toRows(fs.createReadStream(tsvSimple)
			, tsvLoader, options
		...
		it('should use the provided headers', function() {
			...
			return rowStreamer.toRows(fs.createReadStream(tsvMissingHeaders)
			, tsvLoader, options
			, function headerRowCallback(dataRow) {
				headerRow = dataRow;
				++headerCount;
		...
		it('should be able to continue / join lines', function() {
			...
			return rowStreamer.toRows(fs.createReadStream(tsvContinue)
			, tsvLoader, options,
				function headerRowCallback(dataRow) {
					headerRow = dataRow;
					++headerCount;
	...
	describe('#getAttributes()', function() {
		...
		it('get the proper headers & data types back', function() {
			...
			return rowStreamer.getAttributes(fs.createReadStream(tsvSimple)
				, tsvLoader, datasetTransformer, options
		...
		it('get the proper headers & data types back (decimal integers)', function() {
			...
			return rowStreamer.getAttributes(
				fs.createReadStream(tsvDecimalInt)
				, tsvLoader, datasetTransformer, options
			).then(function(fileAttributes) {
	...
	describe('#getData()', function() {
		...
		it('should loop through the dataset entirely and not skip anything', function(done) {
			...
			return rowStreamer.getData(
				fs.createReadStream(tsvSimple)
				, tsvLoader, datasetTransformer, options, false,
				function dataRowCallback(dataRow) { ++dataRowCount; }
			).then(function() {
				dataRowCount.should.equal(4);
			});
		...
		it('should skip empty data rows', function(done) {
			...
			return rowStreamer.getData(
				fs.createReadStream(tsvEmptyRow)
				, tsvLoader, datasetTransformer, options, false,
				function dataRowCallback(dataRow) { ++dataRowCount; }
			).then(function() {
